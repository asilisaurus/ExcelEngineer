...
function processExcelFile(buffer) {
  console.log('=== ОБРАБОТКА ФАЙЛА (ULTIMATE JS VERSION) ===');
...
function extractDataPerfectly(data) {
  const reviews = [];
  const topComments = [];
...
function extractThemeFromText(text) {
  if (!text) return '';
...
function convertToDateFormat(dateValue) {
  if (!dateValue) return '';
...
function createPerfectWorkbook(data) {
  ...
  console.log('✅ Создан лист с именем "Март 2025"');
  
  return wb;
}

function applyPerfectFormatting(ws, totalRows) {
  ...
  console.log('✅ Применено форматирование с объединенными ячейками');
}
